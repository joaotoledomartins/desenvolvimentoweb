...
<body>
    ...
    $pessoasFiltradas = array_filter($pessoas, function($pessoa, $indice){
        ...
        if((!(empty($_GET['nome'])))&&(stripos($pessoa["nome"], $_GET['nome']) > -1)&&(!(empty($_GET['cidade'])))&&(stripos($pessoa["cidade"], $_GET['cidade']) > -1)&&(!(empty($_GET['idade'])))&&($_GET['idade'] == $idade) && (!(empty($_GET['estado']))) &&($_GET['estado'] == $pessoa["estado"])){
            return true;
        }
        return false;
    ...
    $montarTabela = function ($dados) : string {
        $retorno = "<table border='1px solid black'><tr><th>Nome</th><th>Data de Nascimento</th><th>Cidade</th><th>Estado</th></tr>";
    ...
    echo $montarTabela($pessoasFiltradas);
    ?>
</body>
</html>